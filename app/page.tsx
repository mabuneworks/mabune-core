  ...
  if (view === 'list') {
    ...
    return (
      <div className="p-8 bg-slate-50 min-h-screen text-slate-900 font-black">
        <header className="max-w-6xl mx-auto flex justify-between items-center mb-10"><h1 className="text-4xl font-serif tracking-tighter text-slate-800">mabune Core</h1><button onClick={() => { setSelectedId(null); setVisitInfo(initialSession); setBaseInfo({name:'',gender:'',age:'',address:'',phone:'',history:'',surgery:'',romLimit:'',goals:''}); setCurrentTags([]); setView('edit'); }} className="bg-slate-900 text-white px-10 py-4 rounded-full font-black shadow-2xl hover:scale-105 transition">＋ 新規登録</button></header>
      ...
      <main className="p-8 max-w-7xl mx-auto w-full">
        ...
        {activeTab === 'base' && (
          <div className="space-y-12 font-black text-slate-900 animate-in fade-in">
            ...
            <div className="p-10 bg-slate-50 rounded-[3rem] border-2 space-y-4"><label className="text-xs uppercase text-slate-400 tracking-widest font-black">タグ管理 (自由に追加・クリックで削除)</label><div className="flex gap-2 flex-wrap pt-2">{currentTags.map(t => <span key={t} onClick={() => setCurrentTags(currentTags.filter(x => x !== t))} className="bg-blue-700 text-white px-5 py-2 rounded-full cursor-pointer hover:bg-red-500 transition font-bold text-sm">#{t} ✕</span>)}<div className="flex border-2 rounded-full overflow-hidden bg-white border-slate-200"><input value={newTagInput} onChange={e => setNewTagInput(e.target.value)} placeholder="タグ名を入力..." className="px-5 py-2 outline-none font-bold text-slate-700" /><button onClick={addTag} className="bg-slate-900 text-white px-6 font-black hover:bg-black transition">追加</button></div></div></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-16"><div className="space-y-10"><div className="border-b-8 border-slate-900 pb-4">氏名<input value={baseInfo.name} onChange={e=>setBaseInfo({...baseInfo, name:e.target.value})} className="w-full text-7xl font-black bg-transparent outline-none" /></div><div className="grid grid-cols-2 gap-6">
                ...
                <div className="p-8 bg-slate-50 border-2 rounded-[3rem]">年齢<input value={baseInfo.age} onChange={e=>setBaseInfo({...baseInfo, age:e.target.value})} className="w-full bg-transparent text-3xl font-black" /></div></div>
                ...
                <div className="p-8 bg-slate-50 border-2 rounded-[3rem]">電話番号<input value={baseInfo.phone} onChange={e=>setBaseInfo({...baseInfo, phone:e.target.value})} className="w-full bg-transparent text-3xl font-black" /></div></div>
              <div className="space-y-10"><div className="p-12 bg-slate-900 text-white rounded-[4.5rem] shadow-2xl"><h4 className="text-blue-400 text-xl font-black uppercase mb-6 tracking-tighter">なりたい姿 cải thiện</h4><textarea value={baseInfo.goals} onChange={e=>setBaseInfo({...baseInfo, goals:e.target.value})} className="w-full h-80 bg-transparent text-4xl font-serif mt-4 outline-none leading-relaxed" placeholder="想いを記載..." /></div>
                <div className="p-8 bg-slate-50 border-2 rounded-[3rem] space-y-6">
                  ...
                  <div>可動域制限・指示<textarea value={baseInfo.romLimit} onChange={e=>setBaseInfo({...baseInfo, romLimit:e.target.value})} className="w-full h-24 bg-transparent text-xl font-black outline-none" /></div></div></div></div>
          </div>
        )}
      </main>
    </div>
