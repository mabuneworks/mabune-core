...
export default function MabuneUltimateCore() {
  ...
  const generateAllComparisons = async () => {
    ...
    const pairs = [
      { key: 'front', label: '前面', b: visitInfo.images.before.front, a: visitInfo.images.after.front },
      { key: 'side', label: '側面', b: visitInfo.images.before.side, a: visitInfo.images.after.side },
      { key: 'back', label: '背面', b: visitInfo.images.before.back, a: visitInfo.images.after.back },
      { key: 'face', label: '顔', b: visitInfo.images.before.face, a: visitInfo.images.after.face },
    ];

    const newComparisons = { ...visitInfo.images.comparisons };
    alert('比較画像を生成中です。完了までこのままお待ちください...');
  ...
  const openEdit = (p: Patient) => {
    setSelectedId(p.id);
    setBaseInfo({ 
      name: p.name || '', gender: p.base_info?.gender || '', age: p.base_info?.age || '', address: p.base_info?.address || '',
      phone: p.base_info?.phone || '', history: p.base_info?.history || '', surgery: p.base_info?.surgery || '', 
    ...
    if (p.chart_data?.latest) {
      ...
      setVisitInfo({
        ...initialSession, ...L,
        visitNumber: L.visitNumber || (p.chart_data.history?.length + 1) || 1,
  ...
  if (view === 'list') {
    const allUniqueTags = Array.from(new Set(patients.flatMap(p => p.tags || [])));
    const filtered = patients.filter(p => (p.name.includes(searchTerm) || p.base_info?.address?.includes(searchTerm)) && (filterTag ? p.tags?.includes(filterTag) : true));

    return (
      <div className="p-8 bg-slate-50 min-h-screen text-slate-900 font-black">
        <header className="max-w-6xl mx-auto flex justify-between items-center mb-10">
          <h1 className="text-4xl font-serif tracking-tighter text-slate-800">mabune Core</h1>
          <button onClick={() => { setSelectedId(null); setVisitInfo(initialSession); setBaseInfo({name:'',gender:'',age:'',address:'',phone:'',history:'',surgery:'',romLimit:'',goals:''}); setCurrentTags([]); setView('edit'); }} className="bg-slate-900 text-white px-10 py-4 rounded-full font-black shadow-2xl hover:scale-105 transition">＋ 新規ゲスト登録</button>
        </header>

        <div className="max-w-6xl mx-auto mb-8 flex gap-4">
          <input placeholder="氏名・住所で検索..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="flex-grow p-4 rounded-2xl border-2 border-slate-200 outline-none focus:border-blue-600" />
          <select value={filterTag} onChange={e => setFilterTag(e.target.value)} className="p-4 rounded-2xl border-2 border-slate-200 bg-white min-w-[200px] text-slate-500">
        ...
        <div className="max-w-6xl mx-auto grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filtered.map(p => (
            <div key={p.id} onClick={() => openEdit(p)} className="p-8 bg-white rounded-[3rem] shadow-sm border-4 border-transparent hover:border-blue-600 cursor-pointer transition flex flex-col justify-between aspect-[4/3]">
              <div className="text-2xl text-slate-800">{p.name} 様</div>
              <div className="text-blue-700 font-mono text-7xl font-black self-center">{p.chart_data?.latest?.beautyScore ?? '--'}</div>
  ...
  const currentVisitCount = visitInfo.visitNumber;
      ...
      <header className="p-4 border-b-4 flex justify-between items-center sticky top-0 bg-white z-50">
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col border-r-4 pr-6 border-slate-100">
            <input value={baseInfo.name} onChange={e => setBaseInfo({...baseInfo, name: e.target.value})} className="text-2xl font-black bg-transparent outline-none focus:bg-slate-50 rounded px-1 text-slate-800" placeholder="氏名" />
            <div className="flex items-center gap-1 mt-1 text-blue-600">
              <span className="text-[10px] font-black">受診回数: 第</span>
              <input type="number" value={currentVisitCount} onChange={e => setVisitInfo({...visitInfo, visitNumber: parseInt(e.target.value) || 1})} className="w-10 text-center bg-slate-50 rounded font-black text-xs outline-none border border-blue-100" />
              <span className="text-[10px] font-black">回</span>
            </div>
          </div>
        </div>
      ...
      <main className="p-8 max-w-7xl mx-auto w-full">
        {activeTab === 'visit' && (
          <div className="space-y-16 animate-in fade-in">
            <section className="grid grid-cols-1 md:grid-cols-2 gap-8 p-10 bg-slate-50 rounded-[3rem] border-2 border-slate-100">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-20 items-start">
              <div className="space-y-6">
                <div className="flex justify-between items-end"><h3 className="text-3xl border-l-[16px] border-blue-700 pl-6 font-black uppercase tracking-tighter">人体図</h3><button onClick={() => canvasRef.current?.getContext('2d')?.clearRect(0,0,800,1066)} className="bg-slate-200 text-slate-600 px-6 py-2 rounded-full text-xs font-black hover:bg-red-50 hover:text-red-600 transition">記入キャンセル</button></div>
                <div className="relative w-full bg-slate-50 rounded-[4rem] border-4 border-slate-200 overflow-hidden shadow-inner aspect-[3/4] touch-none">
                  <img src="/body-map.png" className="absolute inset-0 w-full h-full object-contain p-12 opacity-40 pointer-events-none" />
                ...
                <div className="space-y-4 max-h-[600px] overflow-y-auto pr-6 border-b-4 pb-10 scrollbar-hide">
                  ...
                  {["肩上","軸","AS","大転子","肘","肩","耳","肩内旋左","肩内旋右"].map(k=>(
                    <div key={k} className="p-6 bg-white rounded-[2rem] border-2 border-slate-100 shadow-sm space-y-4 font-black">
                      <div className="flex justify-between items-center text-xl font-black text-slate-800">{k}<span>{visitInfo.numericInspections[k]?.toFixed(1)}</span></div>
            ...
            <section className="space-y-12 border-t-8 border-slate-100 pt-16 pb-20">
              <div className="flex justify-between items-end"><h3 className="text-3xl border-l-[16px] border-blue-700 pl-6 uppercase tracking-widest font-black">Before & After 分析</h3><button onClick={generateAllComparisons} className="bg-green-600 text-white px-10 py-5 rounded-full text-xl shadow-2xl font-black animate-pulse">✨ 全比較画像を自動生成</button></div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
                {['before', 'after'].map(timing => (
                  <div key={timing} className="space-y-6"><p className={`text-center font-black text-xl tracking-[0.4em] uppercase ${timing === 'before' ? 'text-slate-400' : 'text-blue-700'}`}>{timing}</p>
                    <div className="grid grid-cols-2 gap-4">
                      {['前面', '横', '背面', '顔'].map(pos => {
                        const key = { '前面': 'front', '横': 'side', '背面': 'back', '顔': 'face' }[pos];
                        const currentImg = (visitInfo.images as any)[timing][key as string];
                        return (<div key={pos} onClick={() => { const dummy = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg=="; setVisitInfo({ ...visitInfo, images: { ...visitInfo.images, [timing]: { ...(visitInfo.images as any)[timing], [key as string]: dummy } } }); }} className="aspect-[3/4] bg-slate-50 border-4 border-dashed border-slate-300 rounded-[2rem] flex flex-col items-center justify-center relative cursor-pointer overflow-hidden group">{currentImg ? <img src={currentImg} className="absolute inset-0 w-full h-full object-cover" /> : <span className="text-slate-300 font-black text-xs uppercase">{pos}</span>}<div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white text-[10px]">クリックして入力</div></div>);
                      })}
                    </div>
                  </div>
                ))}
              ...
              {Object.values(visitInfo.images.comparisons).some(img => img) && (
                <div className="mt-16 p-10 bg-slate-900 rounded-[4rem] text-center shadow-inner space-y-10">
                    <p className="text-blue-400 tracking-[0.5em] font-black uppercase text-xs">Generated Comparison Cards</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        {['front','side','back','face'].map(k => (visitInfo.images.comparisons as any)[k] && <div key={k} className="p-4 bg-white rounded-3xl shadow-2xl relative group"><img src={(visitInfo.images.comparisons as any)[k]} className="rounded-xl w-full" /><p className="mt-2 text-slate-900 uppercase font-black text-xs">{k} Card</p><button className="absolute top-4 right-4 bg-green-500 text-white px-4 py-1 rounded-full text-[10px] opacity-0 group-hover:opacity-100 transition">LINE送信</button></div>)}
                    </div>
                </div>
              )}
            </section>
        ...
        {activeTab === 'base' && (
          <div className="space-y-12 font-black text-slate-900 animate-in fade-in">
            ...
            <div className="p-10 bg-slate-50 rounded-[3rem] border-2 space-y-4">
              <label className="text-xs uppercase text-slate-400 tracking-widest font-black">タグ管理 (自由に追加・クリックで削除)</label>
              <div className="flex gap-2 flex-wrap pt-2">
                ...
                <div className="flex border-2 rounded-full overflow-hidden bg-white border-slate-200">
                  <input value={newTagInput} onChange={e => setNewTagInput(e.target.value)} placeholder="タグ名を入力..." className="px-5 py-2 outline-none font-bold text-slate-700" />
                  <button onClick={addTag} className="bg-slate-900 text-white px-6 font-black hover:bg-black transition">追加</button>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
              <div className="space-y-10">
                <div className="border-b-8 border-slate-900 pb-4">氏名<input value={baseInfo.name} onChange={e=>setBaseInfo({...baseInfo, name:e.target.value})} className="w-full text-7xl font-black bg-transparent outline-none text-slate-800" /></div>
                <div className="grid grid-cols-2 gap-6"><div className="p-8 bg-slate-50 border-2 rounded-[3rem]">性別<input value={baseInfo.gender} onChange={e=>setBaseInfo({...baseInfo, gender:e.target.value})} className="w-full bg-transparent text-3xl font-black text-slate-800" /></div><div className="p-8 bg-slate-50 border-2 rounded-[3rem]">年齢<input value={baseInfo.age} onChange={e=>setBaseInfo({...baseInfo, age:e.target.value})} className="w-full bg-transparent text-3xl font-black text-slate-800" /></div></div>
                <div className="p-8 bg-slate-50 border-2 rounded-[3rem]">住所<input value={baseInfo.address} onChange={e=>setBaseInfo({...baseInfo, address:e.target.value})} className="w-full bg-transparent text-3xl font-black text-slate-800" /></div>
                <div className="p-8 bg-slate-50 border-2 rounded-[3rem]">電話番号<input value={baseInfo.phone} onChange={e=>setBaseInfo({...baseInfo, phone:e.target.value})} className="w-full bg-transparent text-3xl font-black text-slate-800" /></div>
              </div>
              <div className="space-y-10">
                <div className="p-12 bg-slate-900 text-white rounded-[4.5rem] shadow-2xl"><h4 className="text-blue-400 text-2xl tracking-widest uppercase mb-6 font-black tracking-tighter">なりたい姿</h4><textarea value={baseInfo.goals} onChange={e=>setBaseInfo({...baseInfo, goals:e.target.value})} className="w-full h-80 bg-transparent text-4xl font-serif mt-4 outline-none leading-relaxed" placeholder="想いを記載してください..." /></div>
                <div className="p-8 bg-slate-50 border-2 rounded-[3rem] space-y-6">
                  <div>既往歴<textarea value={baseInfo.history} onChange={e=>setBaseInfo({...baseInfo, history:e.target.value})} className="w-full h-24 bg-transparent text-xl font-black outline-none border-b border-slate-200" /></div>
                  <div>手術歴<textarea value={baseInfo.surgery} onChange={e=>setBaseInfo({...baseInfo, surgery:e.target.value})} className="w-full h-24 bg-transparent text-xl font-black outline-none border-b border-slate-200" /></div>
                  <div>可動域制限・指示<textarea value={baseInfo.romLimit} onChange={e=>setBaseInfo({...baseInfo, romLimit:e.target.value})} className="w-full h-24 bg-transparent text-xl font-black outline-none" /></div>
                </div>
              </div>
            </div>
          </div>
