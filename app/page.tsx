...
export default function MabuneUltimateCore() {
  ...
  const generateComparison = () => {
    ...
    if (!canvas || !visitInfo.images.before.face || !visitInfo.images.after.face) return alert('Before/Afterの「顔」写真が必要です。');
  ...
  const openEdit = (p: Patient) => {
    ...
    // 古い形式のデータを新しい形式(initialSession)で上書き・補完
    if (p.chart_data?.latest) {
      setVisitInfo({
        ...initialSession,
        ...p.chart_data.latest,
        numericInspections: { ...initialSession.numericInspections, ...(p.chart_data.latest.numericInspections || {}) },
        metaInspections: { ...initialSession.metaInspections, ...(p.chart_data.latest.metaInspections || {}) },
        images: { ...initialSession.images, ...(p.chart_data.latest.images || {}) }
      });
    } else {
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {patients.map(p => (
            <div key={p.id} onClick={() => openEdit(p)} className="p-8 bg-white rounded-[3rem] shadow-sm border-4 border-transparent hover:border-blue-600 cursor-pointer">
              ...
              <div className="mt-2 text-blue-700 font-mono text-xl font-black">Score: {p.chart_data?.latest?.beautyScore ?? '--'}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <header className="p-4 border-b-4 flex justify-between items-center sticky top-0 bg-white z-50">
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col border-r-4 pr-6 border-slate-100">
            ...
            <span className="text-xs text-blue-600 font-bold uppercase mt-1">受診回数: 第{visitCount}回</span>
          </div>
        </div>
        ...
        {activeTab === 'visit' && (
          <div className="space-y-16">
            <section className="grid grid-cols-1 md:grid-cols-2 gap-8 p-10 bg-slate-50 rounded-[3rem] border-2 border-slate-100">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 bg-white/50 border-b-4 border-slate-200 p-2">
                  ...
                  <input type="number" value={visitInfo.amount} onChange={e => setVisitInfo({...visitInfo, amount: parseInt(e.target.value)})} className="w-full text-3xl font-black outline-none bg-transparent font-black" />
                </div>
              </div>
            </section>
              ...
              <div className="space-y-10">
                ...
                <div className="space-y-4 max-h-[700px] overflow-y-auto pr-6 scrollbar-hide border-b-4 pb-10">
                  ...
                  {["肩上","軸","AS","大転子","肘","肩","耳","肩内旋左","肩内旋右"].map(k=>(
                    <div key={k} className="p-6 bg-white rounded-[2rem] border-2 border-slate-100 shadow-sm space-y-4 font-black text-slate-900">
            ...
            <section className="space-y-12 border-t-8 border-slate-100 pt-16 pb-20">
              <div className="flex justify-between items-end"><h3 className="text-3xl border-l-[16px] border-blue-700 pl-6 uppercase tracking-widest font-black text-slate-900">Before & After 分析</h3><button onClick={generateComparison} className="bg-green-600 text-white px-10 py-4 rounded-full text-xl shadow-2xl font-black">✨ 写真を合成して保存</button></div>
        ...
        {activeTab === 'base' && (
          <div className="space-y-12 font-black text-slate-900">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
              ...
              <div className="p-10 bg-slate-900 text-white rounded-[4.5rem] shadow-2xl"><h4 className="text-blue-400 text-xl tracking-widest uppercase mb-6 font-black">ありたい姿</h4><textarea value={baseInfo.goals} onChange={e=>setBaseInfo({...baseInfo, goals:e.target.value})} className="w-full h-80 bg-transparent text-4xl font-serif mt-4 outline-none leading-relaxed" /></div>
            </div>
          </div>
        )}
      </main>
