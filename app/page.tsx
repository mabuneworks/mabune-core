...
export default function MabuneUltimateCore() {
  ...
  if (view === 'list') {
    ...
    const filtered = patients.filter(p => {
      ...
      const matchesTag = filterTag ? tags.includes(filterTag) : true;
    ...
    return (
      <div className="p-8 bg-slate-50 min-h-screen text-slate-900 font-black">
        <header className="max-w-6xl mx-auto flex justify-between items-center mb-10">
          ...
          <button onClick={() => { setSelectedId(null); setVisitInfo(initialSession); setBaseInfo({name:'',gender:'',age:'',address:'',phone:'',history:'',surgery:'',romLimit:'',goals:''}); setCurrentTags([]); setView('edit'); }} className="bg-slate-900 text-white px-10 py-4 rounded-full font-black shadow-2xl hover:scale-105 transition">＋ 新規ゲスト</button>
        </header>
      ...
      <header className="p-4 border-b-4 flex justify-between items-center sticky top-0 bg-white z-50">
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col border-r-4 pr-6 border-slate-100">
            <input value={baseInfo.name} onChange={e => setBaseInfo({...baseInfo, name: e.target.value})} className="text-2xl font-black bg-transparent outline-none focus:bg-slate-50 rounded px-1 text-slate-800" placeholder="氏名を入力" />
            <div className="flex items-center gap-1 mt-1 text-blue-600">
              <span className="text-[10px] font-black uppercase">受診回数: 第</span>
              ...
              <span className="text-[10px] font-black uppercase">回</span>
            </div>
          </div>
        </div>
                ...
                <div className="space-y-4 max-h-[600px] overflow-y-auto pr-6 border-b-4 pb-10 scrollbar-hide">
                  ...
                  {["肩上","軸","AS","大転子","肘","肩","耳","肩内旋左","肩内旋右"].map(k=>(
                    <div key={k} className="p-6 bg-white rounded-[2rem] border-2 border-slate-100 shadow-sm space-y-4 font-black">
                      <div className="flex justify-between items-center text-xl font-black">{k}<span>{visitInfo.numericInspections[k]?.toFixed(1)}</span></div>
                  ...
                  <div key={timing} className="space-y-6"><p className={`text-center font-black text-xl tracking-[0.4em] uppercase ${timing === 'before' ? 'text-slate-400' : 'text-blue-700'}`}>{timing}</p>
                    <div className="grid grid-cols-2 gap-4">
                      {['前面', '横', '背面', '顔'].map(pos => {
                        ...
                        return (<div key={pos} onClick={() => { const dummy = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg=="; setVisitInfo({ ...visitInfo, images: { ...visitInfo.images, [timing]: { ...(visitInfo.images as any)[timing], [key as string]: dummy } } }); }} className="aspect-[3/4] bg-slate-50 border-4 border-dashed border-slate-200 rounded-[2rem] flex flex-col items-center justify-center relative cursor-pointer overflow-hidden group">{currentImg ? <img src={currentImg} className="absolute inset-0 w-full h-full object-cover" /> : <span className="text-slate-300 font-black text-xs uppercase">{pos}</span>}<div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white text-[10px]">クリックして入力</div></div>);
                      })}
                    </div>
                  </div>
                ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
              <div className="space-y-10">
                ...
                <div className="grid grid-cols-2 gap-6">
                    <div className="p-8 bg-slate-50 border-2 rounded-[3rem]">性別<input value={baseInfo.gender} onChange={e=>setBaseInfo({...baseInfo, gender:e.target.value})} className="w-full bg-transparent text-3xl font-black" /></div>
                    <div className="p-8 bg-slate-50 border-2 rounded-[3rem]">年齢<input value={baseInfo.age} onChange={e=>setBaseInfo({...baseInfo, age:e.target.value})} className="w-full bg-transparent text-3xl font-black" /></div>
                </div>
                <div className="p-8 bg-slate-50 border-2 rounded-[3rem]">住所<input value={baseInfo.address} onChange={e=>setBaseInfo({...baseInfo, address:e.target.value})} className="w-full bg-transparent text-3xl font-black" /></div>
                <div className="p-8 bg-slate-50 border-2 rounded-[3rem]">電話番号<input value={baseInfo.phone} onChange={e=>setBaseInfo({...baseInfo, phone:e.target.value})} className="w-full bg-transparent text-3xl font-black" /></div>
              </div>
              <div className="space-y-10">
                <div className="p-12 bg-slate-900 text-white rounded-[4.5rem] shadow-2xl"><h4 className="text-blue-400 text-xl mb-6 uppercase tracking-widest font-black">なりたい姿 cải thiện</h4><textarea value={baseInfo.goals} onChange={e=>setBaseInfo({...baseInfo, goals:e.target.value})} className="w-full h-80 bg-transparent text-4xl font-serif mt-4 outline-none leading-relaxed" /></div>
