...
export default function MabuneUltimateCore() {
  ...
  const generateAllComparisons = async () => {
    ...
    alert('比較画像を自動生成しています。そのままお待ちください...');
  ...
  if (view === 'list') {
    ...
    const filtered = patients.filter(p => {
      const name = p.name || "";
      const addr = p.base_info?.address || "";
      const matchesSearch = name.includes(searchTerm) || addr.includes(searchTerm);
    ...
    return (
      <div className="p-8 bg-slate-50 min-h-screen text-slate-900 font-black">
        ...
        <div className="max-w-6xl mx-auto mb-8 flex gap-4">
          ...
          <select value={filterTag} onChange={e => setFilterTag(e.target.value)} className="p-4 rounded-2xl border-2 border-slate-200 bg-white min-w-[200px] font-black text-slate-500">
        ...
        <div className="max-w-6xl mx-auto grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filtered.map(p => (
            <div key={p.id} onClick={() => openEdit(p)} className="p-8 bg-white rounded-[3rem] shadow-sm border-4 border-transparent hover:border-blue-600 cursor-pointer transition flex flex-col justify-between aspect-[4/3]">
              <div className="text-2xl text-slate-800">{p.name || "名前なし"} 様</div>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col border-r-4 pr-6 border-slate-100">
            ...
            <div className="flex items-center gap-1 mt-1 text-blue-600">
              ...
              <input type="number" value={visitInfo.visitNumber} onChange={e => setVisitInfo({...visitInfo, visitNumber: parseInt(e.target.value) || 1})} className="w-10 text-center bg-slate-50 rounded font-black text-xs outline-none border border-blue-100" />
          ...
          <div className="space-y-16 animate-in fade-in">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-20 items-start">
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-end"><h3 className="text-3xl border-l-[16px] border-blue-700 pl-6 font-black uppercase tracking-tighter">人体図</h3><button onClick={() => canvasRef.current?.getContext('2d')?.clearRect(0,0,800,1066)} className="bg-slate-200 text-slate-600 px-6 py-2 rounded-full text-xs font-black hover:bg-red-100 hover:text-red-600 transition">記入キャンセル (全消去)</button></div>
              ...
              <div className="space-y-10">
                ...
                <div className="space-y-4 max-h-[600px] overflow-y-auto pr-6 border-b-4 pb-10 scrollbar-hide">
                  ...
                  {['首', '腰'].map(k => {
                    const sK = k === '首' ? 'neck' : 'waist';
                    return (<div key={k} className="p-6 bg-slate-50 rounded-[2rem] border-2 border-slate-100 space-y-4 font-black"><label className="text-lg text-slate-800 font-black tracking-tighter">{k}</label><Selector label="左右" current={(visitInfo.metaInspections as any)[sK].side} options={['左', '右']} onSelect={(v: any) => setVisitInfo({...visitInfo, metaInspections: {...visitInfo.metaInspections, [sK]: {...(visitInfo.metaInspections as any)[sK], side: v}}})} /><Selector label="位置" current={(visitInfo.metaInspections as any)[sK].pos} options={['上', '中', '下']} onSelect={(v: any) => setVisitInfo({...visitInfo, metaInspections: {...visitInfo.metaInspections, [sK]: {...(visitInfo.metaInspections as any)[sK], pos: v}}})} /></div>);
                  })}
